...
#define INVALID_OUTPUT do{\
   ...
   char line1[20];
   for(int stage=1; fgets(line1, sizeof line1, fin); ++stage){
      ...
      char *buf = malloc(n+2);
      if(!fgets(buf, sizeof buf, fin)) INVALID_OUTPUT;
...
#define WRONG_ARG do{\
   ...
   if(input && output){
      ...
      rewind(fpo);
      Board puzzle;
      for(int stage=1; Board_read(&puzzle, fpi); ++stage){
         ...
         for(int i=0; i<=n-1; ++i){
            ...
            if(!Board_do_move(&puzzle, strchr(dir, tolower(sol[i]))-dir, 0)){
               ac = false; break;
            }
         }
