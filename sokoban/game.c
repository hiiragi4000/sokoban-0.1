...
Board Brd;
// history of moves
Vector_Move History = {.data=NULL, .size=0, .capacity=0};

// draw the board
// if the flag |clear| is true, erase the previous board
...
restart:
      ...
      while(Brd.m_cnt){
         ...
         switch(cmd){
               ...
               switch(cmd){
                  ...
                  case 't':
                     ...
                     for(int i=History.size-1; i>=0; --i){
                        Board_undo_move(&Brd, History.data[i]);
                     }
                     VM_clear(&History);
                     goto restart;
                  case 'n':
            ...
            case 'u':
               if(History.size){
                  Board_undo_move(&Brd, History.data[History.size-1]);
                  VM_pop_back(&History);
                  dir = 4;
               }
               break;
         }
         if(dir==-1 || (dir!=4 && !Board_do_move(&Brd, dir, &History))){
            putchar('\a');
         }else Draw(true);
      }
